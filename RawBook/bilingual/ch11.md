...
![Figure 11-1. Launching five kernels from the CPU onto the two streams running on the GPU](../images/figure-11-1.png)

![图 11-1. 从 CPU 向 GPU 上运行的两个 stream 启动五个核函数](../images/figure-11-1.png)
...
![Figure 11-2. Timeline of three-way overlap](../images/figure-11-2.png)

![图 11-2. 三路重叠的时间线](../images/figure-11-2.png)
...
![Figure 11-3. Stream-ordered memory allocation](../images/figure-11-3.png)

![图 11-3. 流序内存分配](../images/figure-11-3.png)
...
![Figure 11-4. Timeline showing multiple GPU kernels running concurrently across separate CUDA streams issued from different threads on their respective default streams with PTDS enabled](../images/figure-11-4.png)

![图 11-4. 时间线，展示在启用 PTDS 的情况下，从不同线程在各自默认流上发起、跨相互独立的 CUDA stream 并发运行的多个 GPU 核函数](../images/figure-11-4.png)
...
![Figure 11-5. Using cudaStreamSynchronize() will block the CPU until all operations in the stream have synchronized](../images/figure-11-5.png)

![图 11-5. 使用 cudaStreamSynchronize() 会阻塞 CPU，直到该 stream 中的所有操作都已同步](../images/figure-11-5.png)
...
![Figure 11-6. Using CUDA events to synchronize in a fine-grained manner](../images/figure-11-6.png)

![图 11-6. 使用 CUDA events 进行细粒度同步](../images/figure-11-6.png)
...
![Figure 11-7. Synchronizing CUDA streams with events (source: https://oreil.ly/MynOA)](../images/figure-11-7.png)

![图 11-7. 用 events 同步 CUDA streams（source: https://oreil.ly/MynOA）](../images/figure-11-7.png)
...
![Figure 11-8. Chunked all-reduce with four GPUs in a ring](../images/figure-11-8.png)

![图 11-8. 四 GPU 环形结构中的分块 all-reduce](../images/figure-11-8.png)
...
![Figure 11-9. Using PDL to launch Kernel B from Kernel A—partially overlapping Kernel B’s execution with Kernel A’s epilogue (in the same CUDA stream)](../images/figure-11-9.png)

![图 11-9. 使用 PDL 从 Kernel A 启动 Kernel B——让 Kernel B 的执行与 Kernel A 的收尾（epilogue）部分重叠（在同一 CUDA stream 中）](../images/figure-11-9.png)
...
![Figure 11-10. Warp-specialized, multistage pipeline with PDL and thread block clusters and TMA multicast for a high-performance, inter-kernel and intra-kernel GEMM implementation](../images/figure-11-10.png)

![图 11-10. 结合 PDL、线程块簇与 TMA 多播的 warp 专门化多阶段流水线，用于实现高性能的核间与核内 GEMM](../images/figure-11-10.png)
...
![Figure 11-11. TMA multicast: a leader CTA issues cp.async.bulk.tensor into DSMEM (cluster shared memory); the follower CTAs consume tiles using cluster.map_shared_rank; cuda::memcpy_async drives TMA](../images/figure-11-11.png)

![图 11-11. TMA 多播：主导 CTA 将 cp.async.bulk.tensor 发射到 DSMEM（簇共享内存）；跟随 CTA 使用 cluster.map_shared_rank 消费这些 tile；cuda::memcpy_async 驱动 TMA](../images/figure-11-11.png)
