...
![Figure 9-1. Example Roofline model (GFLOP/s versus arithmetic intensity in FLOPs/byte](../images/figure-9-1.png)

![图 9-1. Roofline 模型示例（GFLOP/s 对以 FLOPs/byte 为单位的算术强度）](../images/figure-9-1.png)
...
![Figure 9-2. Multilevel tiling between global memory (DRAM), shared memory (SMEM), and registers](../images/figure-9-2.png)

![图 9-2. 全局内存（DRAM）、共享内存（SMEM）与寄存器之间的多级分块](../images/figure-9-2.png)
...
![Figure 9-3. DSMEM shared by CTAs (thread blocks) within a CGA](../images/figure-9-3.png)

![图 9-3. 一个 CGA 内的多个 CTA（线程块）共享的 DSMEM](../images/figure-9-3.png)
...
![Figure 9-4. For these four (2 × 2) thread block clusters, each tile of A and B is loaded into four CTAs (thread blocks) simultaneously using multicast (source: https://oreil.ly/EEO_O)](../images/figure-9-4.png)

![图 9-4. 对于这四个（2 × 2）线程块簇，A 和 B 的每个 tile 都通过多播被同时载入四个 CTA（线程块）（来源：https://oreil.ly/EEO_O）](../images/figure-9-4.png)
...
![Figure 9-5. 2:4 structured sparsity](../images/figure-9-5.png)

![图 9-5. 2:4 结构化稀疏](../images/figure-9-5.png)
...
![Figure 9-6. TMEM supports the Tensor Cores by accumulating partial results (instead of registers)](../images/figure-9-6.png)

![图 9-6. TMEM 通过累加部分结果（而非使用寄存器）来支撑 Tensor Core](../images/figure-9-6.png)
...
![Figure 9-7. Mixed-precision and matrix multiply-accumulate (MMA)](../images/figure-9-7.png)

![图 9-7. 混合精度与矩阵乘加（MMA）](../images/figure-9-7.png)
...
![Figure 9-8. Using the asynchronous memory copy instruction (cp.async) to load data from global memory into shared memory without involving the register file and optionally the L1 cache](../images/figure-9-8.png)

![图 9-8. 使用异步内存拷贝指令（cp.async）把数据从全局内存加载到共享内存，而不涉及寄存器堆，也可选地不涉及 L1 缓存](../images/figure-9-8.png)
