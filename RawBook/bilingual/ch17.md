...
![Figure 17-1. Interference caused by colocated prefill and decode running on the same GPU (source: https://oreil.ly/GRkHs)](../images/figure-17-1.png)

![图 17-1. 由同置（colocated）在同一 GPU 上运行的 prefill 与 decode 所引起的干扰（来源：https://oreil.ly/GRkHs）](../images/figure-17-1.png)
...
![Figure 17-2. Comparison between colocated and disaggregated prefill and decode (source: https://oreil.ly/GRkHs)](../images/figure-17-2.png)

![图 17-2. 同置与分离式 prefill 和 decode 之间的对比（来源：https://oreil.ly/GRkHs）](../images/figure-17-2.png)
...
![Figure 17-3. Prefill worker design: read prefix cache → compute prefill → write KV cache for decode](../images/figure-17-3.png)

![图 17-3. prefill 工作节点设计：读取前缀缓存 → 计算 prefill → 为 decode 写入 KV 缓存](../images/figure-17-3.png)
...
![Figure 17-4. Transfer of KV cache data between the prefill and decode workers using NIXL; coalesce multiple PagedAttention blocks into ~128-token payloads before RDMA (note: vLLM defaults to 16 tokens per block on CUDA)](../images/figure-17-4.png)

![图 17-4. 使用 NIXL 在 prefill 与 decode 工作节点之间传输 KV 缓存数据；在 RDMA 之前把多个 PagedAttention 块合并为 ~128-token 的负载（注：vLLM 在 CUDA 上默认每块 16 个 token）](../images/figure-17-4.png)
...
![Figure 17-5. Managing and reusing KV cache data between requests](../images/figure-17-5.png)

![图 17-5. 在请求之间管理并复用 KV 缓存数据](../images/figure-17-5.png)
...
![Figure 17-6. KV-cache-aware request routing based on data received from KV cache events emitted by the workers](../images/figure-17-6.png)

![图 17-6. 基于从工作节点发出的 KV 缓存事件所接收数据的 KV 缓存感知请求路由](../images/figure-17-6.png)
...
![Figure 17-7. NVIDIA’s Dynamo GPU Planner decides how to handle incoming requests and allocate GPU workers to prefill and decode based on GPU utilization metrics](../images/figure-17-7.png)

![图 17-7. NVIDIA 的 Dynamo GPU Planner 根据 GPU 利用率指标决定如何处理传入的请求，并将 GPU 工作节点分配给 prefill 和 decode](../images/figure-17-7.png)
