...
![Figure 10-1. Two-stage producer-consumer pattern with the CUDA Pipeline API](../images/figure-10-1.png)

![图 10-1. 使用 CUDA Pipeline API 的两阶段生产者—消费者模式](../images/figure-10-1.png)
...
![Figure 10-2. Hiding global DRAM load (L) latency with (C) compute using a producer–consumer pipeline](../images/figure-10-2.png)

![图 10-2. 使用生产者—消费者流水线以 (C) 计算隐藏全局 DRAM 加载 (L) 延迟](../images/figure-10-2.png)
...
![Figure 10-3. Nonwarp specialized kernel with each warp performing a mix of both data loading and compute (source: https://oreil.ly/WZDbM)](../images/figure-10-3.png)

![图 10-3. 非 warp 专门化的核函数，每个 warp 都混合执行数据加载与计算（来源：https://oreil.ly/WZDbM）](../images/figure-10-3.png)
...
![Figure 10-4. Warp-specialized kernel with one set of warps for loading data and all other warps for computations (source: https://oreil.ly/WZDbM)](../images/figure-10-4.png)

![图 10-4. warp 专门化核函数，一组 warp 负责加载数据，其余所有 warp 负责计算（来源：https://oreil.ly/WZDbM）](../images/figure-10-4.png)
...
![Figure 10-5. Three-role warp-specialized pipeline configuration with one set of warps for loading data, another set for compute, and another set for data storing (source: https://oreil.ly/xs7YN)](../images/figure-10-5.png)

![图 10-5. 三角色 warp 专门化流水线配置，一组 warp 负责加载数据、另一组负责计算、再一组负责数据存储（来源：https://oreil.ly/xs7YN）](../images/figure-10-5.png)
...
![Figure 10-6. Ping-pong architecture with three-role warp-specialized kernel (source: https://oreil.ly/xs7YN)](../images/figure-10-6.png)

![图 10-6. 采用三角色 warp 专门化核函数的 ping-pong 架构（来源：https://oreil.ly/xs7YN）](../images/figure-10-6.png)
...
![Figure 10-7. Block-wide barrier prevents threads from proceeding until they synchronize and load the new data](../images/figure-10-7.png)

![图 10-7. 块级屏障阻止线程继续执行，直到它们完成同步并加载新数据](../images/figure-10-7.png)
...
![Figure 10-8. Decode throughput improvement with megakernels relative to vLLM and SGLang (source: https://oreil.ly/2aZiF)](../images/figure-10-8.png)

![图 10-8. 巨型核函数相对于 vLLM 和 SGLang 的解码吞吐提升（来源：https://oreil.ly/2aZiF）](../images/figure-10-8.png)
...
![Figure 10-9. Synchronizing across different granularities of threads using cooperative groups](../images/figure-10-9.png)

![图 10-9. 使用协作组在不同粒度的线程之间进行同步](../images/figure-10-9.png)
...
![Figure 10-10. Multiple thread block clusters are guaranteed to be coscheduled on the same GPC or GPC partition](../images/figure-10-10.png)

![图 10-10. 多个线程块簇被保证协同调度到同一个 GPC 或 GPC 分区上](../images/figure-10-10.png)
...
![Figure 10-11. For these four (2 × 2) thread clusters, each tile of A and B is loaded into two thread blocks simultaneously (source: https://oreil.ly/kEZsv)](../images/figure-10-11.png)

![图 10-11. 对于这四个（2 × 2）线程簇，A 和 B 的每个分块被同时加载到两个线程块中（来源：https://oreil.ly/kEZsv）](../images/figure-10-11.png)
...
![Figure 10-12. Thread block swizzling to read tiles A and B in a single wave out of L2 cache](../images/figure-10-12.png)

![图 10-12. 线程块交错，在单一波中从 L2 缓存读取分块 A 和 B](../images/figure-10-12.png)
...
![Figure 10-13. Thread block pair combines two thread blocks](../images/figure-10-13.png)

![图 10-13. 线程块对由两个线程块组合而成](../images/figure-10-13.png)
...
![Figure 10-14. Thread block pair (aka CTA pair) loading tiles as operands for an A * B matrix multiply (source: https://oreil.ly/kEZsv)](../images/figure-10-14.png)

![图 10-14. 线程块对（即 CTA pair）为 A * B 矩阵乘法加载分块作为操作数（来源：https://oreil.ly/kEZsv）](../images/figure-10-14.png)
...
![Figure 10-15. Thread block pair with Tensor Cores and TMEM](../images/figure-10-15.png)

![图 10-15. 配合 Tensor Core 与 TMEM 的线程块对](../images/figure-10-15.png)
...
![Figure 10-16. Sharing data between SMs in a thread block cluster using DSMEM](../images/figure-10-16.png)

![图 10-16. 在一个线程块簇中使用 DSMEM 在 SM 之间共享数据](../images/figure-10-16.png)
...
![Figure 10-17. For these four (2 × 2) thread clusters, each tile is loaded once and multicast into the shared memory of all CTAs in each cluster (source: https://oreil.ly/kEZsv)](../images/figure-10-17.png)

![图 10-17. 对于这四个（2 × 2）线程簇，每个分块只加载一次，并被多播到每个簇中所有 CTA 的共享内存中（来源：https://oreil.ly/kEZsv）](../images/figure-10-17.png)
...
![Figure 10-18. DSMEM uses an SM-to-SM thread block cluster-local network between two thread block clusters for its data exchange](../images/figure-10-18.png)

![图 10-18. DSMEM 使用两个线程块簇之间的一个 SM 间、簇内本地网络进行数据交换](../images/figure-10-18.png)
